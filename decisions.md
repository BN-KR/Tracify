...
## API Key Prefix Renamed to `tracify_sk_live_` (2026-06-16)
- **Decision:** Changed the API key prefix to `tracify_sk_live_`, replacing the prefix carried over from the product's previous name.
- **Files Changed:** `convex/projects.ts`, `src/lib/api-keys.ts`, `src/app/api/ingest/route.ts`, `waiting-step.tsx`, `docs-viewer.tsx`, `quickstart-guide.tsx`.
- **Rationale:** The old prefix was a leftover from the previous project name. All keys generated by Convex after this deploy use `tracify_sk_live_`.
- **Superseded 2026-08-19:** the sitewide rebrand removed acceptance of the old prefix entirely. Only `tracify_sk_live_` authenticates now, so any key issued before the 2026-06-16 change no longer works and must be reissued.

## Google OAuth Credentials Updated (2026-06-16)
- **Decision:** Replaced old Google OAuth credentials with new ones in Vercel production environment.
- **Rationale:** Previous credentials were test/dev values; the new ones are linked to the production Clerk instance.

## Production Clerk Auth Config Deployed to Convex (2026-06-16)
- **Decision:** Deployed `convex/auth.config.ts` with `https://clerk.tracify.tech` provider to Convex production (`focused-otter-289`).
- **Fix:** Resolved "Auth: Waiting" hang on onboarding by ensuring Convex trusts JWTs from the production Clerk instance.
- **Root Cause:** Convex prod only had the old dev Clerk domain (`many-crab-79.clerk.accounts.dev`) configured; it rejected tokens from `clerk.tracify.tech`.

## Sanity CMS Blog Integration (2026-06-15)
- **Decision:** Use Sanity.io as the headless CMS for the blog.
- **Files Created:** `sanity/schemas/post.ts`, `sanity/schemas/blockContent.ts`, `src/lib/sanity/client.ts`, `src/lib/sanity/queries.ts`.
- **Pages Built:** `/blog`, `/blog/[slug]`, `/blog/rss.xml`, `/sitemap.xml`.
- **Sanity Studio:** https://8no3oibu.sanity.studio
- **Rationale:** Free tier, mobile-friendly hosted Studio, rich text editing out of the box. Not embedded in the Next.js build to avoid bloat.

## Google OAuth Credentials Updated (2026-06-16)
- **Decision:** Replaced old Google OAuth client ID/secret with new production credentials in `.env.prod` and Vercel env.
...
- **Status:** Added.
- **Decision:** After the pain (DebugStream) and the setup (FirstTrace), the user needs to see exactly what "observability" looks like in tracify.
...
- **Status:** Integrated.
- **Decision:** Do not use the provided component template blindly; it required adaptation for tracify's strict monochrome and 0px radius design system.
