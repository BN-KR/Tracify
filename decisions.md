...
- **Decision:** Changed API key prefix from `5t1r_sk_live_` to `tracify_sk_live_`.
...
- **Rationale:** The `5t1r` prefix was a leftover from the old project name. All new keys generated by Convex after this deploy will use `tracify_sk_live_`. Existing keys with the `5t1r_sk_live_` prefix continue to be accepted by the ingest API.
