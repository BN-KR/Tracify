...
export default function PricingPage() {
  ...
  return <FuturePage>
    <FutureMasthead eyebrow="Pricing / Operating scale" title={<>Pay when the record becomes operational.</>} description="Start with real traces. Upgrade when your team needs deeper retention, evaluation, collaboration, and release controls." index="P01" aside={<div><p className="font-mono text-[8px] uppercase tracking-[0.13em] text-white/40">Billing mode</p><div className="mt-5 flex border border-white/25 p-1"><button type="button" onClick={() => setAnnual(false)} aria-pressed={!annual} className={`flex-1 px-3 py-2 font-mono text-[8px] uppercase ${!annual ? "bg-[#f4d44d] text-black" : "text-white/50"}`}>Monthly</button><button type="button" onClick={() => setAnnual(true)} aria-pressed={annual} className={`flex-1 px-3 py-2 font-mono text-[8px] uppercase ${annual ? "bg-[#f4d44d] text-black" : "text-white/50"}`}>Annual −20%</button></div></div>} />
