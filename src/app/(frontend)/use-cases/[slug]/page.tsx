...
export default async function UseCasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <FuturePage>
    <header className={`border-b border-black ${caseIndex === 1 ? "bg-black text-white" : caseIndex === 2 ? "bg-[#f4d44d]" : "bg-[#eceae3]"}`}><div className="mx-auto max-w-[1240px] border-x border-current"><div className={`grid min-h-[540px] md:grid-cols-[minmax(0,1fr)_380px] ${reverse ? "md:grid-cols-[380px_minmax(0,1fr)]" : ""}`}>
      <div className={`flex flex-col justify-between px-5 py-12 sm:px-8 md:px-10 md:py-16 ${reverse ? "md:order-2 md:border-l" : "md:border-r"}`}><p className="font-mono text-[9px] uppercase tracking-[0.15em] opacity-50">Use case / {page.title}</p><h1 className="font-pixel text-[clamp(4rem,8vw,7.5rem)] leading-[0.8] tracking-[-0.075em]">{page.statement}</h1><p className="max-w-xl text-base leading-7 opacity-60">{page.description}</p></div>
      <div className={`relative flex items-center justify-center overflow-hidden border-t border-current md:border-t-0 ${reverse ? "md:order-1" : ""} ${caseIndex === 1 ? "bg-[#f4d44d] text-black" : "bg-black text-white"}`}><span className="font-pixel text-[15rem] leading-none tracking-[-0.12em]">{caseMarks[caseIndex]}</span><div className="absolute inset-x-6 bottom-6 border-t border-current/30 pt-3 font-mono text-[8px] uppercase tracking-[0.13em]">Failure pattern / 0{caseIndex + 1}</div></div>
    </div></div></header>
