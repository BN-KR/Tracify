  ...
  return <FuturePage>
    <header className="border-b border-black bg-black text-white">
      <div className="mx-auto max-w-[1240px] border-x border-white/20">
        ...
        <div className="grid md:grid-cols-[1fr_340px]"><div className="px-5 py-14 sm:px-8 md:px-10 md:py-20"><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-white/35">Public operations board</p><h1 className="mt-8 font-pixel text-[clamp(4rem,9vw,8rem)] leading-[0.8] tracking-[-0.075em]">No green dot without evidence.</h1></div><div className="flex flex-col justify-between border-t border-white/20 p-6 md:border-l md:border-t-0 md:p-8"><div className="grid grid-cols-7 gap-2" aria-hidden="true">{Array.from({length:28}).map((_,i)=><span key={i} className={`aspect-square ${i > 24 ? "bg-[#f4d44d]" : "bg-white/10"}`}/>)}</div><p className="mt-14 text-sm leading-6 text-white/50">Automated incident history is being connected. Until then, support remains the honest source of truth.</p></div></div>
      </div>
    </header>
