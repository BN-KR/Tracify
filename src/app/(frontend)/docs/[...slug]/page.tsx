...
export default async function DocsPage({
  ...
  return <FuturePage>
    <header className={`border-b border-black ${chapterIndex % 3 === 0 ? "bg-black text-white" : chapterIndex % 2 === 0 ? "bg-[#f4d44d]" : "bg-[#eceae3]"}`}><div className="mx-auto max-w-[1240px] border-x border-current"><div className="grid md:grid-cols-[120px_1fr_320px]">
      <div className="flex min-h-28 items-center justify-center border-b border-current font-pixel text-6xl opacity-25 md:min-h-[500px] md:border-b-0 md:border-r"><span className="md:-rotate-90">D{String(chapterIndex).padStart(2,"0")}</span></div>
      <div className="flex flex-col justify-between px-5 py-12 sm:px-8 md:px-10 md:py-16"><p className="font-mono text-[9px] uppercase tracking-[0.15em] opacity-50">Documentation / {docKey}</p><h1 className="my-16 font-pixel text-[clamp(4rem,8vw,7.5rem)] leading-[0.8] tracking-[-0.07em]">{page.title}</h1><p className="max-w-xl text-base leading-7 opacity-60">{page.description}</p></div>
      <aside className={`relative flex min-h-72 items-center justify-center overflow-hidden border-t border-current md:border-l md:border-t-0 ${chapterIndex % 3 === 0 ? "bg-[#f4d44d] text-black" : "bg-black text-white"}`}><span className="select-none font-pixel text-[clamp(7rem,16vw,13rem)] leading-none tracking-[-0.1em] opacity-90">{motifs[docKey] ?? "DOC"}</span><div className="absolute inset-x-5 bottom-5 flex justify-between border-t border-current/25 pt-3 font-mono text-[8px] uppercase tracking-[0.12em]"><span>Read</span><span>Run</span><span>Verify</span></div></aside>
    </div></div></header>
    {example ? <>
