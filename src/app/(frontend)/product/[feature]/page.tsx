...
export default async function ProductFeaturePage({ params }: { params: Promise<{ feature: string }> }) {
  ...
  return <FuturePage>
    <header className={`border-b border-black ${isDark ? "bg-black text-white" : featureIndex % 3 === 1 ? "bg-[#f4d44d]" : "bg-[#eceae3]"}`}><div className="mx-auto max-w-[1240px] border-x border-current"><div className={`grid min-h-[420px] ${featureIndex % 2 ? "md:grid-cols-[300px_1fr]" : "md:grid-cols-[1fr_300px]"}`}>
      <div className={`relative flex min-h-52 items-center justify-center overflow-hidden p-8 ${featureIndex % 2 ? "md:order-1" : "md:order-2"} ${isDark ? "bg-[#f4d44d] text-black" : "bg-black text-white"}`}><span className="select-none font-pixel text-[clamp(6rem,14vw,11rem)] leading-none tracking-[-0.08em]">{motifs[featureIndex]}</span><span className="absolute left-5 top-5 font-mono text-[8px] uppercase tracking-[0.14em]">Instrument 0{featureIndex + 1}</span></div>
      <div className={`flex flex-col justify-between border-t border-current px-5 py-10 sm:px-8 md:border-t-0 md:px-10 md:py-12 ${featureIndex % 2 ? "md:order-2 md:border-l" : "md:order-1 md:border-r"}`}><p className="font-mono text-[9px] uppercase tracking-[0.15em] opacity-50">Product surface / {feature}</p><h1 className="my-8 font-pixel text-[clamp(3rem,6vw,5.5rem)] leading-[0.88] tracking-[-0.055em]">{title}</h1><p className="max-w-xl text-base leading-7 opacity-70">{description}</p></div>
    </div></div></header>
