...
function SessionsSurface({ workspace, basePath, recordId, onReadOnly }: { workspace: DashboardWorkspace; basePath: string; recordId?: string; onReadOnly?: (action: string) => void }) {
  ...
  return <main className="captured-sessions-surface">
    ...
    <div className="captured-sessions-layout"><aside className="captured-sessions-filter-rail"><button type="button" aria-label="Open session filters">⌕</button><button type="button" aria-label="Expand session filters">⌄</button><button type="button" aria-label="Select session filters">□</button><button type="button" aria-label="More session filters">›</button></aside><div className="captured-sessions-table-wrap"><table><thead><tr><th> </th><th>ID</th><th>Created At ▼</th><th>Environment</th><th>User ID</th><th>Trace Count</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td><Link href={`${basePath}/sessions/${encodeURIComponent(record.id)}`} aria-label={`Open ${record.id}`}>›</Link></td><td><Link href={`${basePath}/sessions/${encodeURIComponent(record.id)}`}>{record.id}</Link></td><td>{record.timestamp}</td><td>{record.environment}</td><td>{record.userId ?? "—"}</td><td>{record.score ?? "—"}</td></tr>)}</tbody></table></div></div><footer className="captured-table-footer"><span>Rows <strong>50</strong></span><span>Page 1 of 1</span><button type="button" disabled>‹</button><button type="button" disabled>›</button></footer>
  </main>;
}
