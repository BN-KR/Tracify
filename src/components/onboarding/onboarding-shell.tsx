...
const stepPresentation = {
  ...
  waiting: { code: "04", label: "Listen for evidence", note: "The ingest channel is open. Run the instrumented agent to send its first span.", icon: Radio, tone: "bg-black text-white" },
    ...
    <main className="relative min-h-svh bg-[#eceae3] font-mono text-black">
      ...
      <div className="mx-auto grid min-h-[calc(100svh-54px)] max-w-[1440px] lg:grid-cols-[minmax(320px,0.72fr)_minmax(0,1.28fr)]">
        <aside className={`flex min-h-80 flex-col justify-between border-b border-black p-6 sm:p-8 md:p-10 lg:min-h-0 lg:border-b-0 lg:border-r ${presentation.tone}`}>
          <div className="flex items-start justify-between gap-6"><span className="font-pixel text-7xl leading-none tracking-[-0.08em] opacity-20">{presentation.code}</span><StepIcon className="size-9" strokeWidth={1.2} aria-hidden="true" /></div>
          <div className="my-12"><p className="font-mono text-[9px] uppercase tracking-[0.16em] opacity-55">Onboarding / active step</p><h1 className="mt-5 font-pixel text-5xl leading-[0.88] tracking-[-0.065em] md:text-7xl">{presentation.label}</h1><p className="mt-6 max-w-md font-sans text-sm leading-6 opacity-65">{presentation.note}</p></div>
          <OnboardingProgress currentStep={currentStep} />
        </aside>
        <section className="flex items-center px-5 py-10 sm:px-8 md:px-12 md:py-16">
          <div className="w-full max-w-[760px] border border-black bg-[#111111] text-white">
...
export function OnboardingHeader({
  ...
  return (
    <div className="mb-8 border-b border-white/20 pb-6">
      <h2 className="font-pixel text-4xl leading-none tracking-[-0.055em] text-white">
        {title}
      </h2>
