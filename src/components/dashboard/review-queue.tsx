...
export function ReviewQueue({ projectId }: { projectId: string }) {
  ...
  return <div className="space-y-6">
    ...
    {message ? <p className="border border-black/15 p-3 text-xs text-black/60">{message}</p> : null}
    <section className="grid gap-3 md:grid-cols-4">{[["Queued", count("queued")], ["In review", count("in_review")], ["Completed", count("completed")], ["Agreement", agreement?.agreement === null || agreement?.agreement === undefined ? "—" : `${Math.round(agreement.agreement * 100)}%`]].map(([labelValue, value]) => <div key={String(labelValue)} className="border border-black/15 p-3"><p className="font-mono text-[10px] uppercase text-black/55">{labelValue}</p><p className="mt-2 font-mono text-lg text-black">{value}</p></div>)}</section>
    <section className="space-y-2">{annotations?.map((annotation) => { const annotationReviews = reviews?.filter((review) => review.annotationId === annotation._id) ?? []; return <div key={annotation._id} className="border border-black/15 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><Link href={`/dashboard/${projectId}/runs/${annotation.traceId}`} className="font-mono text-sm text-black underline decoration-black/30 underline-offset-4 hover:decoration-black">{annotation.traceId}</Link><span className="ml-3 font-mono text-[10px] uppercase text-black/55">{annotation.status}</span>{annotation.assignee ? <span className="ml-3 font-mono text-[10px] uppercase text-black/55">assigned:{annotation.assignee}</span> : null}</div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={async () => { await claim({ projectId: projectId as never, annotationId: annotation._id }); setReviewing(annotation._id); setMessage("Review claimed. Add an independent label or score."); }}>Claim</Button>{annotation.status === "queued" ? <Button size="sm" variant="outline" onClick={() => void update({ projectId: projectId as never, annotationId: annotation._id, status: "in_review" })}>Start</Button> : null}</div></div><p className="mt-2 text-sm text-black/60">{annotation.notes || "No notes yet."}</p>{annotationReviews.length ? <div className="mt-3 space-y-1 border-t border-black/15 pt-3">{annotationReviews.map((review) => <p key={review._id} className="font-mono text-[10px] text-black/55">{review.reviewerName} · {review.status}{review.label ? ` · ${review.label}` : ""}{review.score === undefined ? "" : ` · ${review.score.toFixed(2)}`}</p>)}</div> : null}{reviewing === annotation._id ? <div className="mt-4 grid gap-2 border-t border-black/15 pt-3 md:grid-cols-[1fr_120px_2fr_auto]"><Input value={reviewLabel} onChange={(event) => setReviewLabel(event.target.value)} placeholder="Your label" /><Input value={reviewScore} onChange={(event) => setReviewScore(event.target.value)} placeholder="Score 0–1" type="number" min="0" max="1" step="0.01" /><Input value={reviewNotes} onChange={(event) => setReviewNotes(event.target.value)} placeholder="Independent notes" /><Button onClick={() => void submit(annotation._id)}>Submit</Button></div> : null}</div>; })}</section>
  </div>;
}
