...
function DocsNavigation({ docs, activeSlug }: { docs: DocsSidebarItem[]; activeSlug?: string }) {
  ...
  return <nav className="border-t border-black px-5 pb-6 lg:mt-8 lg:border-t-0 lg:px-0">{sections.map((section) => <div key={section} className="border-b border-black/20 py-5 last:border-b-0"><p className="font-mono text-[9px] uppercase tracking-[.14em] text-black/45">{section}</p><div className="mt-3 space-y-2">{docs.filter((doc) => doc.section === section).map((doc) => <Link key={doc.slug} href={`/docs/${doc.slug}`} className={`block text-sm leading-5 ${doc.slug === activeSlug ? "font-medium underline underline-offset-4" : "text-black/65 hover:text-black"}`}>{doc.title}</Link>)}</div></div>)}</nav>;
}
