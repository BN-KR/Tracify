...
## Conventions
- **Design (marketing site only — homepage, `/pricing`, `/product/[feature]`, shared nav/footer):** a **white/light base** (`#FFFFFF` content backgrounds, `#F2F2F3`/`#E0E0E1` subtle section backgrounds) with a **full multi-color accent system**, not a navy-dominant two-tone. Rotate accents through the palette — yellow `#FFF133`/`#FFF67D`, orange `#FA4028`/`#FA7560`, pink `#FF80C8`/`#FFB3DE`, purple `#7C5AC4`/`#391E7E`/`#E7E3F7`, green `#45FF00`/`#96FF6F`/`#D2FFC1`, blue `#0095FF`/`#1378D1`/`#CEEBFF` — for chips, badges, highlights, section dividers, and hover states. Deep navy `#00063D` is reserved for headers/nav text and the occasional dark panel, not a background-filling block; neutral gray `#5E5D5F` for secondary/supporting text. **Corners are square (0px radius) everywhere** — the `--mkt-radius-*` tokens in the `.marketing-rebrand` scope (`globals.css`) are all `0`, matching Tracify's original 0px-radius convention; do not reintroduce `rounded-*` treatments on marketing surfaces. The Tracify wordmark (`BrandLogo`, `src/components/brand-logo.tsx`) renders in yellow (`#F5D400`) on the marketing site only, via a `.marketing-rebrand span[aria-label="Tracify"]` CSS override in `globals.css` — the shared component itself is untouched, so the dashboard/onboarding/auth usages keep their existing black/white logo. Geist Pixel Square for logos/H1s, Geist Mono for UI/data, Geist Sans for prose. Tailwind v4 + shadcn/ui (`base-nova`) remain the component base — only theme tokens/CSS variables and component styling changed. This palette is scoped to marketing surfaces via the `.marketing-rebrand` wrapper class in `site-chrome.tsx` / `globals.css`, not applied globally — **the authenticated dashboard (`src/app/(frontend)/dashboard/**`) keeps its existing monochrome/0px-radius design system**, do not carry this palette into dashboard, onboarding, billing, or auth screens.
