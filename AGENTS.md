...
## Conventions
- **Design (marketing site only — homepage, `/pricing`, `/product/[feature]`, shared nav/footer):** Jasper-inspired: deep navy `#00063D` as the primary dark (nav, footer, dark panels, CTA buttons), warm orange accent `#FA4028` (replaces the old acid yellow), white `#FFFFFF` content-section backgrounds, neutral gray `#5E5D5F` for secondary/supporting text. Rounded corners with a real radius scale (`--mkt-radius-sm: 0.5rem`, `--mkt-radius-md: 1rem`, `--mkt-radius-lg: 1.5rem`, `--mkt-radius-pill: 999px` — see the `.marketing-jasper` scope in `globals.css`), bold large-type hero, more visual energy than the old flat/square treatment. Geist Pixel Square for logos/H1s, Geist Mono for UI/data, Geist Sans for prose. Tailwind v4 + shadcn/ui (`base-nova`) remain the component base — only theme tokens/CSS variables and component styling changed. This palette is scoped to marketing surfaces via the `.marketing-jasper` wrapper class in `site-chrome.tsx` / `globals.css`, not applied globally — **the authenticated dashboard (`src/app/(frontend)/dashboard/**`) keeps its existing monochrome/0px-radius design system**, do not carry this palette into dashboard, onboarding, billing, or auth screens.
