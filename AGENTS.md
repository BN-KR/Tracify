...
## Conventions
- **Design (marketing site only — homepage, `/pricing`, `/product/[feature]`, shared nav/footer):** Jasper-inspired, corrected against the real jasper.ai site: a **white/light base** (`#FFFFFF` content backgrounds, `#F2F2F3`/`#E0E0E1` subtle section backgrounds) with a **full multi-color accent system**, not a navy-dominant two-tone. Rotate accents through the verified jasper.ai palette — yellow `#FFF133`/`#FFF67D`, orange `#FA4028`/`#FA7560`, pink `#FF80C8`/`#FFB3DE`, purple `#7C5AC4`/`#391E7E`/`#E7E3F7`, green `#45FF00`/`#96FF6F`/`#D2FFC1`, blue `#0095FF`/`#1378D1`/`#CEEBFF` — for chips, badges, highlights, section dividers, and hover states. Deep navy `#00063D` is reserved for headers/nav text and the occasional dark panel, not a background-filling block; neutral gray `#5E5D5F` for secondary/supporting text. **Corners are square (0px radius) everywhere** — the `--mkt-radius-*` tokens in the `.marketing-jasper` scope (`globals.css`) are all `0`, matching Tracify's original 0px-radius convention and jasper.ai's own unrounded layout; do not reintroduce `rounded-*` treatments on marketing surfaces. The Tracify wordmark (`BrandLogo`, `src/components/brand-logo.tsx`) renders in yellow (`#F5D400`) on the marketing site only, via a `.marketing-jasper span[aria-label="Tracify"]` CSS override in `globals.css` — the shared component itself is untouched, so the dashboard/onboarding/auth usages keep their existing black/white logo. Geist Pixel Square for logos/H1s, Geist Mono for UI/data, Geist Sans for prose. Tailwind v4 + shadcn/ui (`base-nova`) remain the component base — only theme tokens/CSS variables and component styling changed. This palette is scoped to marketing surfaces via the `.marketing-jasper` wrapper class in `site-chrome.tsx` / `globals.css`, not applied globally — **the authenticated dashboard (`src/app/(frontend)/dashboard/**`) keeps its existing monochrome/0px-radius design system**, do not carry this palette into dashboard, onboarding, billing, or auth screens.
...
## Markdoc Blog Rules
- **Required skill:** Read `.agents/skills/tracify-blog-tool/SKILL.md` before writing, editing, reviewing, storing, or publishing any blog post or documentation.
- **Per-post quality gate:** Every new or materially rewritten blog post must pass `npm run validate:blog -- <slug>`. This targeted gate enforces the approved editorial composition, unique images, lower-half visual pacing, five-item late FAQ, and practical ending; a generic repository test is not a substitute.
...
- **Canonical presentation gate:** Word count alone is never completion. Every published post also needs a unique article-specific hero, at least two unique teaching visuals distributed through the body, practical or clearly labeled illustrative code, semantic tables, mid-article decision notes, multiple instructional editorial panels, restrained highlights throughout both halves, exactly one late FAQ with five questions, and one purposeful deterministic interaction.
...
- Start new articles from `.agents/skills/tracify-blog-tool/assets/blog-post.mdoc`; do not copy a published post’s structure or media. Required frontmatter: `title`, `slug`, `excerpt`, `publishedAt` (ISO timestamp), and `author`. Keep `categories`, `tags`, and `relatedPosts` as string arrays; related posts reference slugs.
...
- For every new article, include exactly one purposeful interaction at the point of need. Use an approved centralized Markdoc interaction or implement and validate a new centralized type first. Keep interactions deterministic, accessible, mobile-safe, and isolated from secrets and production data.
...
- Before completing any new or materially rewritten blog post, run `npm run validate:blog -- <slug>`, `npm run test:content`, and `npm run build`. Duplicate slugs, malformed frontmatter, invalid Markdoc, missing required fields, or accidental draft exposure are release blockers.
