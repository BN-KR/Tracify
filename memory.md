...
## 2026-09-08 — Explore playground parity and regional handoff

- Rebuilt `/playground` as a public, deterministic synthetic Explore workspace. It no longer waits for Convex auth, reads account-scoped projects, or redirects through the sign-in loop. It carries the stable demo identity `usr_demo_7f3a9c21` only as route/UI context and never uses the stale project ID.
- Reused the captured populated-dashboard information architecture with Tracify branding: dark cloud shell, gray skeleton/placeholder treatment, project selector labeled `Demo Project (view only)`, populated Home metrics/chart/run cards, Tracing table, scenario controls, filters, trace detail, range/environment selectors, and Build CTAs.
- Made every visible synthetic sidebar destination remain on `/playground?view=...`; the Tracify logo returns to `/dashboard` project selection. Build remains the only path that enters regional onboarding.
- Changed Cloud Explore to open the populated playground directly. Build continues to open `/cloud/region`, and region selection now enters `/cloud/connecting` with a visible regional handoff status before the API redirect can cross hosts.
- Added focused browser coverage for the public Explore journey, synthetic nav/control logic, no-Langfuse/no-stale-project contract, Build region handoff, auth route forms, invalid project route handling, and mobile keyboard access. Account-access contract: 8/8 green; TypeScript, focused ESLint, and production Next build (210 pages) green.
- Live follow-up found and fixed one host-boundary issue: the marketing host's relative `/playground` URL redirected back through `/cloud`, so production Explore now targets `https://eu.cloud.tracify.tech/playground?...` directly while local development remains relative. Merged as `312a4df` (PR #100).
- Live verification after the production deployment: `www.tracify.tech/cloud` exposes the EU Explore destination, clicking it reaches the EU synthetic populated Home, and the EU production deployment for `312a4df` is Ready. The PR preview deployment still fails because the Vercel Preview environment lacks non-secret `CONVEX_SITE_URL`; production has it and succeeds.
